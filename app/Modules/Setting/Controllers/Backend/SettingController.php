...
class SettingController extends \App\Http\Controllers\AdminController
{
    ...
    public function postSites(Request $request)
    {
        ...
        if ($request->sites) {
            ...
            Setting::where('key', 'sites')->update(['value' => json_encode($sites)]);
        ...
        if ($request->contact_support || $request->seo_metadata) {
            ...
            Setting::where('key', 'contact_support')->update(['value' => json_encode($request->contact_support)]);
            Setting::where('key', 'seo_metadata')->update(['value' => json_encode($request->seo_metadata)]);
        }
        // Frontend Support
        if ($request->frontend_support) {
            ...
            Setting::where('key', 'frontend_support')->update(['value' => json_encode($frontend_support)]);
        }
        // Payment Gateway
        if ($request->payment_gateway) {
            $request->session()->flash('account-tab', 'payment-gateway');
            Setting::where('key', 'payment_gateway')->update(['value' => json_encode($request->payment_gateway)]);
                ...
                foreach ($stripe_pacakges_all as $id => $name) {
                    if (in_array($id, array_keys($stripe_pacakges))) {
                        StripeProduct::where('id', '=', $id)
                            ->update(['is_active' => $stripe_pacakges[$id]['is_active']]);
                    } else {
                        StripeProduct::where('id', '=', $id)->update(['is_active' => 0]);
                    }
                }
            }
        }
        ...
        if ($request->social_links) {
            ...
            Setting::where('key', 'social_links')->update(['value' => json_encode($request->social_links)]);
        }
        if ($request->announcements) {
            ...
            Setting::where('key', 'announcements')->update(['value' => json_encode($announcements)]);
        }
        // Developer Self Rule Policy
        if ($request->developers) {
            $request->session()->flash('account-tab', 'developer-option');
            Setting::where('key', 'developers')->update(['value' => json_encode($request->developers)]);
        }
